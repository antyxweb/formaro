/* Порт cabinet-html/assets/js/support.js.
   Изменения относительно прототипа:
   - tickets.json был одним объектом {marketplace_contacts, tickets} —
     теперь это два независимых источника: dsLoad('tickets') (реальные
     обращения партнёра, HL-блоки CabinetTickets/CabinetTicketMessages) и
     dsLoad('marketplace_contacts') (статичные реквизиты площадки, см.
     TicketRepository::getMarketplaceContacts() — не сущность с CRUD);
   - создание обращения теперь отдельный AJAX-экшен create_ticket вместо
     dsSave(whole-object) — сервер сам генерирует id и первое сообщение. */
...
$(function () {
    ...
    dsLoad('marketplace_contacts').done(renderContacts);
...
function finishCreateTicket(subject, message, files) {
    ...
    function done() {
        cabinetAjax({
            ...
            window.location.href = CABINET_URL + 'support/' + ticket.id + '/';
        });
    }
...
function renderTickets() {
    ...
    var rows = allTickets.filter(function (t) {
