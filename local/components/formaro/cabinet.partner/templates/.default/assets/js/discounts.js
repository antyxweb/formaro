/* Порт cabinet-html/assets/js/discounts.js.
   Изменения относительно прототипа:
   - discounts.json хранил скидки и купоны одним объектом {discounts, coupons}
     только из-за localStorage — теперь это два независимых entity
     ('discounts'/'coupons'), каждый обычный плоский массив; saveDiscountsData
     (комбинированное сохранение) заменено на два отдельных saveOwnRows;
   - ссылки на *.html — на SEF-роуты компонента. */
...
$(function () {
    ...
    dsLoad('categories').done(function (cats) {
        ...
        dsLoad('products').done(function (prods) {
            ...
            dsLoad('discounts').done(function (rows) {
                allDiscounts = ownOnly(rows);
                renderDiscounts();
            });
            dsLoad('coupons').done(function (rows) {
                allCoupons = ownOnly(rows);
                renderCoupons();
            });
        });
    });
    ...
    $('#dBulkDeleteBtn').on('click', function () {
        ...
        showConfirm('Удалить выбранные скидки (' + ids.length + ')?', function () {
            ...
            saveOwnRows('discounts', null, allDiscounts, function () { showResult(true, 'Скидки удалены'); renderDiscounts(); });
    ...
    $('#cBulkDeleteBtn').on('click', function () {
        ...
        showConfirm('Удалить выбранные купоны (' + ids.length + ')?', function () {
            ...
            saveOwnRows('coupons', null, allCoupons, function () { showResult(true, 'Купоны удалены'); renderCoupons(); });
        }, {danger: true, okText: 'Удалить'});
    });
});
...
function renderDiscountsPage(rows) {
    ...
    rows.forEach(function (d) {
        $t.append(
            ...
            '<td><a href="' + CABINET_URL + 'discounts/edit/' + d.id + '/">' + esc(d.name) + '</a></td>' +
...
function deleteDiscount(id) {
    ...
    showConfirm('Удалить скидку «' + (d ? d.name : '') + '»?', function () {
        ...
        saveOwnRows('discounts', null, allDiscounts, function () { showResult(true, 'Скидка удалена'); renderDiscounts(); });
    }, {danger: true, okText: 'Удалить'});
}
...
function renderCouponsPage(rows) {
    ...
    rows.forEach(function (c) {
        $t.append(
            ...
            '<td><a href="' + CABINET_URL + 'discounts/coupons/edit/' + c.id + '/" class="font-monospace">' + esc(c.code) + '</a></td>' +
            ...
            '<td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="' + CABINET_URL + 'discounts/coupons/edit/' + c.id + '/" title="Редактировать"><svg class="ic-inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"></path><path d="m15 5 4 4"></path></svg></a> ' +
...
function deleteCoupon(id) {
    ...
    showConfirm('Удалить купон «' + (c ? c.code : '') + '»?', function () {
        ...
        saveOwnRows('coupons', null, allCoupons, function () { showResult(true, 'Купон удалён'); renderCoupons(); });
    }, {danger: true, okText: 'Удалить'});
}
