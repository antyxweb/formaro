...
var appliedDiscount = null; // {id, name, discount_type, value, target_type, target_ids, min_qty, min_amount}
var appliedCoupon = null;   // {code, discount_type, value}

function isOrderLocked() { return LOCKED_STATUSES.indexOf($('#statusSelect').val()) !== -1; }

/** При статусе "Подтверждён" и далее по цепочке (Отправлен/Выполнен) состав
    товаров и данные покупателя менять больше нельзя — блокируем поля.
    Статус/доставку/оплату/историю по-прежнему можно вести. */
...
$(function () {
    ...
    isNewOrder = (idParam === 'new');
    orderId = isNewOrder ? 0 : (parseInt(idParam, 10) || 0);
    loadPartials('orders', isNewOrder ? 'Новый заказ' : 'Заказ');
    ...
    dsLoad('discounts', 'data/discounts.json').done(function (data) {
        ...
        // Заказы грузим ПОСЛЕ скидок/купонов — иначе, если у заказа уже была
        // применена скидка/купон, нечем будет восстановить выбор (гонка загрузок)
...
/** Для старых заказов, у которых в самом item ещё нет цвета/размера (создавались
    до этого поля), подсматриваем их у товара по product_id — как подстраховку. */
...
/** Считает подытог/скидку/итого по текущим позициям заказа и применённой
    скидке ИЛИ купону (только один из двух — комбинировать нельзя). Для
    скидки с целью "Категории"/"Товары" скидка считается только с той части
    заказа, что попадает под условие, и только если выполнены условия по
    минимальному количеству/сумме именно по этой части. */
/** Считает сумму скидки d при текущем составе заказа — вынесено отдельно,
    чтобы одной и той же логикой проверять и уже применённую скидку, и
    перебирать все активные скидки при автоподборе (см. autoApplyBestDiscount). */
...
/** Автоподбор скидки при изменении состава заказа: из всех активных скидок
    партнёра выбирается та, что при текущей корзине даёт наибольшую сумму
    скидки и выполняет свои условия (от количества/суммы). Купон в это не
    вмешивается — это явный код, который менеджер вводит вручную. */
...
function renderItems() {
    ...
    currentItems.forEach(function (it, idx) {
        ...
        $b.append(
            '<tr>' +
            '<td>' + (live ? '<a href="product-detail.html?id=' + live.id + '">' + esc(it.name) + '</a>' : esc(it.name)) + ' ' +
    ...
    // Bootstrap иногда "забывает" скрыть popover, если увести курсор быстрым
    // движением между соседними строками — подстраховываемся принудительным
    // скрытием всех попапов при уходе курсора с таблицы целиком.
...
/** Поиск товаров для добавления в заказ — от 3 символов, с имитацией
    задержки Ajax-запроса (данные уже подгружены один раз через dsLoad,
    но в реальной системе здесь будет $.getJSON к серверу с постраничной
    выдачей — поэтому поиск устроен как асинхронный, а не как фильтр select). */
...
function doSaveOrder() {
    ...
    if (isNewOrder) {
        ...
        payload.order_number = 'F-' + (100200 + payload.id);
        payload.created_at = new Date().toISOString();
        payload.history = [{date: payload.created_at, text: 'Заказ создан вручную в панели партнёра', author: 'Менеджер'}];
        allOrders.push(payload);
        dsSave('orders', allOrders);
        showResult(true, 'Заказ создан');
        setTimeout(function () { window.location.href = 'order-detail.html?id=' + payload.id; }, 700);
    } else {
