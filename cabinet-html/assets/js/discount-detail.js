...
$(function () {
    loadPartials('discounts', 'Скидка');

    var idParam = getQueryParam('id');
    discountId = (idParam && idParam !== 'new') ? parseInt(idParam, 10) : 0;
    ...
    dsLoad('categories', 'data/categories.json').done(function (cats) {
        ...
        dsLoad('products', 'data/products.json').done(function (prods) {
            ...
            dsLoad('discounts', 'data/discounts.json').done(function (data) {
                ...
                if (discountId) {
                    var d = allDiscounts.find(function (x) { return x.id === discountId; });
...
/** Переключает пикер "на что действует скидка": дерево категорий (переиспользуем
    компонент из товара) или список выбранных товаров с поиском-добавлением
    (по образцу меток/вариантов товара). */
        ...
        $c.append(
            '<div class="variant-item">' +
              ...
              '<div class="variant-info">' +
                '<div class="variant-name">' + (p ? '<a href="product-detail.html?id=' + p.id + '">' + esc(p.name) + '</a>' : ('#' + id)) + '</div>' +
...
function doSave(goBack) {
    ...
    if (discountId) {
        allDiscounts = allDiscounts.map(function (d) { return d.id === discountId ? Object.assign({}, d, payload) : d; });
    } else {
        payload.id = dsNextId(allDiscounts);
        payload.created_at = new Date().toISOString();
        allDiscounts.push(payload);
    ...
    dsLoad('discounts', 'data/discounts.json').done(function (full) {
        ...
        dsSave('discounts', {discounts: foreignDiscounts.concat(allDiscounts), coupons: full.coupons || []});
        showResult(true, idParamWasNew() ? 'Скидка создана' : 'Скидка сохранена');
        if (goBack) setTimeout(function () { window.location.href = 'discounts.html'; }, 700);
    });
}
function idParamWasNew() { return !getQueryParam('id') || getQueryParam('id') === 'new'; }

function doDelete() {
    if (!discountId) return;
    var d = allDiscounts.find(function (x) { return x.id === discountId; });
    showConfirm('Удалить скидку «' + (d ? d.name : '') + '»? Это действие необратимо.', function () {
        allDiscounts = allDiscounts.filter(function (x) { return x.id !== discountId; });
        dsLoad('discounts', 'data/discounts.json').done(function (full) {
            var foreignDiscounts = (full.discounts || []).filter(function (x) { return x.partner_id !== CURRENT_PARTNER_ID; });
            dsSave('discounts', {discounts: foreignDiscounts.concat(allDiscounts), coupons: full.coupons || []});
            showResult(true, 'Скидка удалена');
            setTimeout(function () { window.location.href = 'discounts.html'; }, 700);
        });
