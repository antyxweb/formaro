...
$(function () {
    loadPartials('dashboard', 'Главная');

    dsLoad('orders', 'data/orders.json').done(function (rows) {
        allOrders = rows;
        // "Сегодня" в демо-данных отсчитывается от даты самого свежего заказа,
        // чтобы фильтр в "Топ товаров" оставался осмысленным в любой день просмотра прототипа
...
function renderTopProducts(rows) {
    ...
    list.forEach(function (p) {
        var rowAttrs = p.product_id ? ' class="cursor-pointer" onclick="location.href=\'product-detail.html?id=' + p.product_id + '\'"' : '';
...
function renderRecentOrders(rows) {
    ...
    rows.forEach(function (o) {
        $t.append(
            '<tr class="cursor-pointer" onclick="location.href=\'order-detail.html?id=' + o.id + '\'">' +
