...
function renderPage(rows) {
    ...
    rows.forEach(function (o) {
        $t.append(
            ...
            '<td><a href="order-detail.html?id=' + o.id + '">' + esc(o.order_number) + '</a></td>' +
            '<td>' + esc(o.customer.name) + '</td>' +
            '<td>' + fmtMoney(o.total) + '</td>' +
            '<td>' + statusPill(o.status) + '</td>' +
            '<td class="text-muted-2 small">' + fmtDate(o.created_at) + '</td>' +
            '<td class="text-end">' +
              '<a class="btn btn-sm btn-outline-secondary" href="order-detail.html?id=' + o.id + '" title="Подробнее"><i class="bi bi-eye"></i></a> ' +
